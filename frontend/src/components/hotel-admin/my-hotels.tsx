...
import { useMyHotels } from "@/hooks/use-get-my-hotels";
import { useDeleteHotel } from '@/hooks/use-delete-hotel';
import { useToggleHotelActive } from '@/hooks/use-toggle-hotel-active';
...
export const MyHotels = () => {
  ...
  const { data: hotels = [], isLoading: isLoadingHotels, error, refetch } = useMyHotels();
  const { deleteHotel, isLoading: isDeleting } = useDeleteHotel();
  const { toggleHotelActive, isLoading: isToggling } = useToggleHotelActive();
  ...
  const handleDelete = async (hotel: Hotel) => {
    if (window.confirm(`Are you sure you want to delete ${hotel.name}?`)) {
      ...
      if (success) {
        // Clear expanded card if the deleted hotel was expanded
        if (expandedCard === hotel.id) {
  ...
  const handleToggleActive = async (hotel: Hotel) => {
    const updatedHotel = await toggleHotelActive(hotel.id, !hotel.is_active);
    if (updatedHotel) {
      refetch(); // Refresh the hotels list after successful toggle
    }
  };
  const closeModal = () => {
    setModalState({
    ...
    // Refetch hotels when modal closes (in case a hotel was created/edited)
    refetch();
  };
                ...
                <div className="absolute top-full left-0 right-0 mt-2 bg-card backdrop-blur-xl border border-border rounded-2xl shadow-2xl p-6 animate-fade-in max-w-lg z-10">
                  ...
                  <div className="flex gap-2 pt-4">
                    ...
                    >
                      ...
                      {isToggling ? 'Updating...' : (hotel.is_active ? 'Deactivate' : 'Activate')}
                    </Button>
                    <Button
                      variant="ghost"
                      size="sm"
    ...
    {!isLoadingHotels && filteredHotels.length === 0 && (
      <div className="text-center py-12">
        ...
        <p className="text-muted-foreground">
          {hotels.length === 0 ? 'Create your first hotel to get started' : 'Try adjusting your search criteria'}
        </p>
      </div>
    )}
